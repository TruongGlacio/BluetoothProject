...
public class ActivityMap extends AppCompatActivity implements BeaconConsumer
{
...
/*    private class TouchHandler implements View.OnTouchListener
    ...
    public void setLocalAndroid() {
        ...
        float ImmuctieuY=(ImMuctieuTouchY-400)/DonviY;
        if(ImmuctieuY<AndroidY) {
            mImageViewDanHuong.setBackground(getResources().getDrawable(R.drawable.muiten_len));
            if(Math.abs(AndroidY-ImmuctieuY)<=Math.abs(AndroidX-ImmuctieuX))
            {
                if(ImmuctieuX<AndroidX)
                {
        ...
        else
        {
            ...
            if(Math.abs(AndroidY-ImmuctieuY)>=Math.abs(ImmuctieuX-AndroidX))
            {
                if(ImmuctieuX<AndroidX)
                {
                    mImageViewDanHuong.setBackground(getResources().getDrawable(R.drawable.muiten_trai));
    ...
    public boolean dispatchTouchEvent(MotionEvent event) {
        ...
        mImageViewMuctieu.setY(ImMuctieuTouchY-400);
        VitrimuctieuthucteX=ImMucTieuTouchX/DonviX;
        VitrimuctieuthucteY=(ImMuctieuTouchY-400)/DonviY;
